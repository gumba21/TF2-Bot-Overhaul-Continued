...
public void OnPluginStart()
{
	...
	cvarDirectory = CreateConVar("sm_botnames_directory", "configs/botnames.txt", "the directory for the bot names list.");
...
void ReloadNamesFromFile()
{
//	char path[PLATFORM_MAX_PATH];
//	BuildPath(Path_SM, path, sizeof(path), BOT_NAME_FILE);

	char dbname[128];
	GetConVarString(cvarDirectory, dbname, sizeof(dbname));
	char path[PLATFORM_MAX_PATH];
	BuildPath(Path_SM, path, sizeof(path), dbname);
...
void ReloadNamesFromDatabase()
{
	...
	// construct our query
	char dbquery[512];
	// we WANT int here -- initializes empty strings
	char orderpart[512];
	char limitpart[512];
...
void DoBotName(int client, int primary)
{
...
public void OnMapStart()
{
	...
	// Showin changed this to just check if its enabled instead of checking the configs are loaded.
	// We need this to always attempt to reload so changing the name directory actually works.
...
// Showin added mvm check here!
public int ClientCheck(int client)
{
	if (IsClientConnected(client) && client > 0)
	{
		if (IsFakeClient(client) && !IsClientReplay(client) && !IsClientSourceTV(client) && MVM == 0 || IsFakeClient(client) && !IsClientReplay(client) && !IsClientSourceTV(client) && MVM == 1 && GetClientTeam(client) == 2)
		{
			return true;
		}
		else
